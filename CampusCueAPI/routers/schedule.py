...
async def get_my_subscriptions(current_user: User = Depends(get_current_user)):
    query = user_schedule_table.select().where(
        user_schedule_table.c.user_id == current_user.id
    )
    rows = await database.fetch_all(query)
    return [row["schedule_item_id"] for row in rows]
...
async def update_course_subscriptions(
    ...
    if not requested_ids:
        wipe_query = user_schedule_table.delete().where(
            user_schedule_table.c.user_id == current_user.id
        )
        await database.execute(wipe_query)
        return

    select_query = schedule_items_table.select().where(
        schedule_items_table.c.id.in_(requested_ids)
    )
    selected_courses = await database.fetch_all(select_query)

    sorted_courses = sorted(
        selected_courses, key=lambda c: (c["day_of_week"], c["start_time"])
    )
    for i in range(len(sorted_courses) - 1):
        ...
        if (
            current_course["day_of_week"] == next_course["day_of_week"]
            and next_course["start_time"] < current_course["end_time"]
        ):
            raise HTTPException(
...
async def create_schedule_override(
    override_data: ScheduleOverride, current_user: User = Depends(get_current_user)
):
    logger.info(
        f"User {current_user.email} creating/updating override for {override_data.override_date}"
    )
    ...
    if existing_override:
        ...
        update_query = (
            schedule_overrides_table.update()
            .where(schedule_overrides_table.c.id == existing_override["id"])
            .values(target_day=override_data.target_day)
        )
