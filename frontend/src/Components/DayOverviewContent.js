...
import { Clock, BookOpen, Utensils, Coffee, FlaskConical, Check, ChefHat, RefreshCw } from 'lucide-react';
...
const updateMessPreference = async (preference) => {
  const { data } = await apiClient.patch('/users/me/mess-preference', preference);
  return data;
};
...
export default function DayOverviewContent({ dayOffset = 0, showTimeAndStatus = true }) {
  ...
  const messPreferenceMutation = useMutation({
    mutationFn: updateMessPreference,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['myMenu', dateStr] });
    },
  });
  ...
  const getCurrentHour = () => currentTime.getHours() + currentTime.getMinutes() / 60;
  ...
  return (
    <div className="space-y-8">
      {showTimeAndStatus && (
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center cursor-pointer" onMouseDown={handleHoldStart} onMouseUp={handleHoldEnd} onMouseLeave={handleHoldEnd} onTouchStart={handleHoldStart} onTouchEnd={handleHoldEnd} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
          <div className="inline-flex items-center gap-3 px-6 py-3 glass rounded-full border border-cyan-400/30 neon-glow-blue">
            ...
            <span className="text-2xl font-mono font-bold text-cyan-300">{currentTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
          </div>
        </motion.div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} drag="x" dragConstraints={{ left: 0, right: 0 }} onDragEnd={handleMenuSwipe} whileDrag={{ scale: 1.02 }} className="space-y-6 cursor-grab active:cursor-grabbing">
        <div className="flex items-center justify-center gap-4 text-center">
          <h2 className="text-2xl font-bold bg-gradient-to-r from-orange-400 to-pink-500 bg-clip-text text-transparent">{dayOffset === 0 ? "Today's Menu" : "Tomorrow's Menu"}</h2>
          {dayOffset === 0 && (
            <motion.button whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }} onClick={toggleCycle} className={`w-4 h-4 rounded-full transition-all ${menuData?.cycle === 'weeks_1_3' ? 'bg-purple-400' : 'bg-pink-400'}`} />
          )}
        </div>
      ...
      {scheduleWithStatus.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} drag="x" dragConstraints={{ left: 0, right: 0 }} onDragEnd={handleScheduleSwipe} whileDrag={{ scale: 1.02 }} className="space-y-6 cursor-grab active:cursor-grabbing">
          <div className="flex items-center justify-center gap-4">
            <h2 className="text-2xl font-bold text-center bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">{dayOffset === 0 ? "Today's Schedule" : "Tomorrow's Schedule"}</h2>
            <motion.button whileHover={{ scale: 1.2, rotate: 180 }} whileTap={{ scale: 0.9 }} onClick={() => setShowOverrideModal(true)} className={`p-2 rounded-full transition-all ${scheduleData?.has_override ? 'bg-orange-500/30 border border-orange-400 text-orange-400' : 'glass border border-gray-600/50 text-gray-400 hover:text-blue-400'}`} title="Override schedule"><RefreshCw className="w-5 h-5" /></motion.button>
          </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 font-medium text-sm">{item.time}</p>
                      </div>
                    </div>
